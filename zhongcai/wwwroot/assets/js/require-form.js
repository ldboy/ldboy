define(['jquery', 'bootstrap', 'upload', 'validator'], function ($, undefined, Upload, Validator) {
    var Form = {
        ...
        events: {
            validator: function (form, success, error, submit) {
                ...
                form.validator($.extend({
                    ...
                    valid: function (ret) {
                        ...
                        var that = this, submitBtn = $("[type=submit]", form);
                        ...
                        var submitfun = function () {
                            ...
                            Form.api.submit($(ret), function (data, ret) {
                                ...
                                WsInit.onAjaxGetData(ret, (ret.data && ret.data['in_this'])?true:false);
                        ...
                        if (submitBtn.hasClass("btn-submit-tip")) {
                            var data = submitBtn.data();
                            Layer.confirm(
                                "<div style='font-size: xx-large'>" + ( data.tip ? data.tip : "确定要提交吗?") + "</div>",

                                {icon:  data.icon!=undefined?data.icon:3, title: data.title ? data.title : "提示", shadeClose: true,area: [300, 300]},
                                function (index) {
                                    Layer.close(index);
                ...
                //移除提交按钮的disabled类
                // alert( $(".layer-footer [type=submit],.fixed-footer [type=submit],.normal-footer [type=submit]", form).size());
            ...
            wsmore: function (form) {
                if ($(".wsbtnmore", form).size() > 0) {

                    $(".wsbtnmore", form).on("click", function () {

                        $(this).parent().prev().append($(this).parent().next().children().get(0).innerHTML);
                    });
                }
        ...
        api: {
            submit: function (form, success, error, submit) {
                ...
                Fast.api.ajax({
                    ...
                    if (data && typeof data === 'obj  ect' && typeof data.token !== 'undefined') {
                        $("input[name='__token__']", form).val(data.token);
                    }
                    if (typeof error === 'function') {
