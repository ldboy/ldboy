        ...
        index: function () {
            ...
            if (Config.referer) {
                if (Config.referer === $(addtabs).attr("url")) {
                    ...
                    //刷新页面后跳到到刷新前的页面
                    Backend.api.addtabs(Config.referer, Backend.api.query("title", Config.referer));
                }
            }
            ...
            function setup() {
                ...
                // 菜单栏展开或收起
                $("[data-enable='expandOnHover']").on('click', f);
            ...
            function shuaxin() {
                $.ajax({
                    ...
                    url: 'http://zcw.wszx.cc/admin/Inform/informlist',
                    ...
                    success: function (ret) {
                        ...
                        $(".notifications-menu  ul > li >span.inform-num").text(ret.new > 0 ? ret.new : 0);
                        $("#zhzheng").text(ret.new > 0 ? ret.new : 0);

                        $("#inform-zhz").text(ret.new > 0 ? ret.new : '无');
                        //$(".notifications-menu .footer a").attr("href", ret.url);
                        $.each(ret.newslist, function (i, j) {

                            var item = '<li><a data-url="' + j.url + '" data-title="' + j.title + '"  href="javascript:;" class="btn-msg"><i class="' + j.class + '">查看</i> <font size="3px">' + j.content + '</font></a></li>';
                            $(item).appendTo($(".notifications-menu ul.menu"));
                        });


                        $(".btn-msg").on("click", function () {
                            Backend.api.addtabs($(this).data("url"), $(this).data("title"));//
                        });
            ...
            $("#top_shuaxin").on("click", function (e) {
                e.stopPropagation();
