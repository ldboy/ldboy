...
class Email
{
     ...
     * @return $this
     */
    public function to($email, $name = '', $idid = 0)
    {


        $this->tousers[] = array("to" => $email, "to_name" => $name, "idid" => $idid);

        return $this;
    }

    public function addUser($email, $name = '',$idid=0)
    {
    ...
    public function addWebAttachment($weburl, $filename)
    {
        $this->mail->addStringAttachment((file_get_contents($weburl)), $filename);
        return $this;
    }
    ...
    public function send()
    {
        ...
        switch ($this->options['mail_type']) {
            case 1:
                ...
                foreach ($this->tousers as $value) {
                    $this->mail->addAddress($value['to'], $value['to_name']);
                }
