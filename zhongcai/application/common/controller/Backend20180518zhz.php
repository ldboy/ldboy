...
class Backend extends Controller
{
    ...
    private  $use_action_js = false;


    public function use_action_js($jsaction=null,$jscontoler=null,$app="backend")
    {
        $jsaction = $jsaction?$jsaction:strtolower($this->request->action());
        $this->use_action_js = ($jscontoler?strtolower($jscontoler):strtolower($this->request->controller()))."/".strtolower($jsaction);
        $this->use_action_js  =$app."/".str_replace('.', '/', $this->use_action_js);
        $this->assignconfig("actionname","run");
        $this->assignconfig("jsname",$this->use_action_js);

    }
    public function use_show_Js()
    {

        $this->assignconfig("actionname","run");
        $this->use_action_js("show", "show", "wscommon");

    }
    public function use_form_Js()
    {
    ...
    public function  useLayout($layout="default")
    {
        // 如果有使用模板布局
        if($layout)
        {
    ...
    public function _initialize()
    {
        ...
        if($admin['id']==Constant::Admin_Role_zhongcaiyuan){
            ...
            $site_['name'] = '仲裁管理系统';
            $admin_ = Db::name('dossier_users')->where("idid",LoginUser::getIdid())->find();
            ...
            $admin_ = Db::name('third_user')->where("th_id",$site_['id'])->find();

        }elseif($admin['id']==Constant::Admin_Role_zhongcaiwei){
            $jg_id = Db::name('jigou_zcy')->where("idid",LoginUser::getIdid())->find();

            $site_ = Db::name('jigou')->where("id",$jg_id['jg_id'])->find();
            $admin_ = Db::name('jigou_admin')->where("id",$jg_id['zcy_id'])->find();
        }
        ...
        $config = [
            ...
            'actionname'     => $this->use_action_js?"run":$actionname,
            'jsname'         =>  $this->use_action_js?$this->use_action_js:('backend/' .str_replace('.', '/', $controllername)),
