            ...
            foreach ($list as $k => $v) {
                if ($v['gid'] == $pid) {
                    ...
                    if ($v['sub_type'] == 0) {
                        ...
                        // $item->required();
                    }else if ($v['sub_type'] == 5) {

                        $item = new Item();
                        $item->varName($v['flag'])->varTitle($v['name'])->inputType(InputType::textarea);
                    }elseif($v['sub_type']==6){
                        $item = new Item();
                        $item->varName($v['flag'])->varTitle($v['name'])->inputType(InputType::datetime)->varType('date');
                    }
