...
//        exit;
        try {
            // Get blob.
            file_put_contents("filename1.txt",1,FILE_APPEND);
//            exit;
            $blob = $blobRestProxy->getBlob(self::$rongqi, $pathtofilename);
            file_put_contents("filename2.txt",$blob,FILE_APPEND);
...
//            $tmpfile = NAS_WSZX_DIR."tmp/".str_replace("/","_",$pathtofilename);
//            if(strstr($pathtofilename,'.png.pdf',true)){
//                if(strlen($name) > 24){
//                    $name = substr($name,-24);
//                }
//                $tmpfile = NAS_WSZX_DIR."tmp/".$name.".png.pdf";
//            }else{
                $tmpfile = NAS_WSZX_DIR."tmp/".$pathtofilename;
//            }

            file_put_contents("filename.txt",$tmpfile,FILE_APPEND);
